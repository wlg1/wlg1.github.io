...
This procedure is similar to genes giving a prior set of skills to an organism, or memes giving a set way of making analogies between new cases and previously learned patterns. This implies that some inheritable parts may be more beneficial than others in certain environments, leading to an evolution of motifs. But if we do not allow neural networks to figure out which motifs are the best and transfer them to others horizontally (like in viruses [cite horizontal gene transfer]) or vertically (like having children), then the neural networks won’t be able to learn from each other. The key is figuring out WHICH motifs to pass on, and how to stitch them together in another neural network.
